...
import { IndianRupee, TrendingUp, User, Users } from "lucide-react";
import { useMemo } from "react";
import type { Expense } from "../backend";
import { useGetExpenses } from "../hooks/useQueries";
import { formatDate, getCategoryKind } from "../lib/categoryUtils";
...
export function RepayReport() {
  ...
  const repayGroups = useMemo((): RepayGroup[] => {
    ...
    const repayExpenses = allExpenses.filter((exp) => {
      ...
      return (
        kind === "cashTransfer" && exp.subCategory === "Repay" && exp.repayName
      );
    });
          ...
          <div className="py-14 flex flex-col items-center gap-3 text-center px-4">
            ...
            <div>
              <p className="font-semibold text-foreground text-sm">
                No repay entries yet
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                Add a Cash Transfer expense with "Repay" sub-category to track
                who owes you money
              </p>
            </div>
          </div>
