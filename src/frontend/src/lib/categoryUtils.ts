...
export const CATEGORY_BADGE_CLASS: Record<CategoryKind, string> = {
  ...
  others: "badge-others",
  creditCard: "badge-creditcard",
  haircut: "badge-haircut",
  donation: "badge-donation",
};

export const FOOD_SUB_CATEGORIES = ["Biryani", "Junk Food", "Tiffin", "Others"];
export const SHOPPING_SUB_CATEGORIES = [
  "Clothes",
  "Accessories",
  "Watches",
  "Skincare",
...
export const TRIP_SUB_CATEGORIES = ["Travel", "Food", "Shopping"];
export const MEDICAL_SUB_CATEGORIES = ["Medicine", "Hospital"];
export const CASH_TRANSFER_SUB_CATEGORIES = ["Family", "NeedToPay", "Repay"];

/** Virtual categories map to 'others' in the backend; the real label goes into subCategory */
export function isVirtualCategory(kind: CategoryKind): boolean {
  return kind === "creditCard" || kind === "haircut" || kind === "donation";
}

export function getVirtualCategorySubLabel(kind: CategoryKind): string {
  if (kind === "creditCard") return "Credit Card";
  if (kind === "haircut") return "Haircut";
  if (kind === "donation") return "Donation";
  return "";
}

/** Returns true when a note is required for the given category/sub-category combination */
export function isNoteRequiredForSubCategory(
  categoryKind: CategoryKind,
  subCategory: string,
): boolean {
  if (categoryKind === "others") return true;
  if (categoryKind === "food" && subCategory === "Others") return true;
  return false;
}
...
export function getSubCategoriesByKind(kind: CategoryKind): string[] {
  ...
  if (kind === "trip") return TRIP_SUB_CATEGORIES;
...
export function hasSubCategoriesByKind(kind: CategoryKind): boolean {
  return ["food", "shopping", "trip", "medical", "cashTransfer"].includes(kind);
}
...
export function buildCategory(
  ...
  // Virtual categories (creditCard, haircut, donation) → store as 'others'
  ...
  switch (kind) {
    ...
    case "medical": {
      const medSub =
        subCategory?.toLowerCase() === "hospital"
    ...
    case "cashTransfer": {
      ...
      if (subCategory?.toLowerCase() === "needtopay")
        ctSub = CashTransferCategory.needToPay;
      else if (subCategory?.toLowerCase() === "repay")
        ctSub = CashTransferCategory.repay;
